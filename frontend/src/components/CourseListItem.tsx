...
import { CourseRecord, DualRatingSummary } from '../services/reviewsService';
...
export const CourseListItem: React.FC<Props> = ({ course, rating, onPress }) => {
  const r = rating || { rating: 0, ratingCount: 0, secondary: 0, secondaryCount: 0, myRating: 0, mySecondary: 0 };

  return (
    <TouchableOpacity style={styles.row} activeOpacity={0.7} onPress={onPress}>
      <View style={{ flex: 1 }}>
        <Text style={styles.title} numberOfLines={1}>{course.nombre || course.codigo}</Text>
