...
import { moviesService, MovieItem, DiscoverMovieProfile } from '../services/moviesService';
import { tmdbService, TmdbResult } from '../services/tmdbService';
...
export const PeliculasScreen: React.FC<Props> = ({ navigation }) => {
  const { user } = useAuth();
  const [activeTab, setActiveTab] = useState<'descubrir' | 'mis-peliculas' | 'matches'>('descubrir');

  // --- Mis Películas: se necesita en más de una pestaña (para saber si ya se puede dar
  // like en Descubrir), así que vive a nivel de pantalla.
  const [myItems, setMyItems] = useState<MovieItem[]>([]);
  ...
  const fetchMyItems = async () => {
    ...
    try {
      ...
      setMyItems(await moviesService.listMyItems(user.id));
    } catch (err) {
      console.error('Error cargando mis películas:', err);
    } finally {
  ...
  // --- Pestaña Descubrir ---
  const [discoverProfiles, setDiscoverProfiles] = useState<DiscoverMovieProfile[]>([]);
  ...
  const fetchDiscover = async () => {
    try {
      ...
      const feed = await moviesService.getDiscoverFeed();
      setDiscoverProfiles(feed);
      setCurrentIndex((prev) => (feed.length > 0 ? prev % feed.length : 0));
    } catch (err) {
      console.error('Error cargando descubrimiento de películas:', err);
    } finally {
  ...
  const handleToggleLike = async () => {
    ...
    if (!target.isLiked && myItems.length === 0) {
      Toast.show({
        ...
        text2: 'Sube al menos un título en "Mis Títulos" antes de dar like.',
      });
      return;
    }
    ...
    try {
      if (target.isLiked) {
        if (target.likeId) await moviesService.deleteLike(target.likeId);
      ...
      await moviesService.createLike(user.id, target.user, true);
      ...
      const match = await moviesService.getMatchBetweenUsers(idA, idB).catch(() => null);
      if (match) {
  ...
  const fetchProfile = async () => {
    if (!user) return;
    const profile = await moviesService.getProfileByUserId(user.id);
  ...
  useFocusEffect(
    useCallback(() => {
      if (activeTab === 'mis-peliculas') fetchProfile();
      // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [activeTab, user?.id])
  );
  ...
  const handleSaveDescription = async () => {
    ...
    try {
      ...
      if (profileId) {
        await moviesService.updateProfile(profileId, { description: description.trim() });
      } else {
        const created = await moviesService.createProfile({ user: user.id, description: description.trim() });
  ...
  const openNewItemForm = () => {
    if (myItems.length >= 5) {
      Toast.show({ type: 'info', text1: 'Límite alcanzado', text2: 'Ya subiste el máximo de 5 películas o series.' });
      return;
    }
    setAddingItem(true);
    setSearchQuery('');
  ...
  const handleSearchTmdb = async () => {
    ...
    setSearching(true);
    try {
      setSearchResults(await tmdbService.search(q));
    } catch (err: any) {
  ...
  const handleSelectResult = async (result: TmdbResult) => {
    ...
    try {
      setSavingItem(true);
      await moviesService.createItem({
        user: user.id,
        title: result.title,
  ...
  const handleDeleteItem = (item: MovieItem) => setDeleteTarget(item);

  const confirmDeleteItem = async () => {
    if (!deleteTarget) return;
    try {
      await moviesService.deleteItem(deleteTarget.id);
      fetchMyItems();
    } catch (err) {
  ...
  const fetchMatches = async () => {
    ...
    try {
      ...
      const res = await moviesService.getMatchesList(user.id);
      setMatches(res.filter((m: any) => m.status !== 'unmatched'));
    } catch (err) {
      console.error('Error cargando matches de películas:', err);
    } finally {
  ...
  useEffect(() => {
    const sub = DeviceEventEmitter.addListener('onGlobalRefresh', async () => {
      await withMinimumDelay(async () => {
        ...
        else if (activeTab === 'mis-peliculas') await Promise.all([fetchMyItems(), fetchProfile()]);
  ...
  const confirmUnmatch = async () => {
    ...
    await moviesService.unmatch(unmatchTarget.id, user.id);
  ...
  return (
    <View style={styles.container}>
      <View style={styles.tabHeader}>
        ...
        <TouchableOpacity style={[styles.tabBtn, activeTab === 'mis-peliculas' && styles.tabBtnActive]} onPress={() => setActiveTab('mis-peliculas')}>
          <Feather name="film" size={18} color={activeTab === 'mis-peliculas' ? theme.colors.primary : '#a3a3a3'} />
          <Text style={[styles.tabBtnText, activeTab === 'mis-peliculas' && styles.tabBtnTextActive]}>Mis Títulos</Text>
        </TouchableOpacity>
      ...
      {activeTab === 'descubrir' &&
        (loadingDiscover ? (
          ...
          <View style={styles.emptyContainer}>
            <Feather name="film" size={40} color={theme.colors.textMuted} style={{ marginBottom: 12 }} />
            <Text style={styles.emptyText}>Todavía nadie ha compartido sus películas o series.</Text>
          </View>
        ) : (
          <PeliculaDiscoverCard
        ...
        (loadingMyItems ? (
          ...
          <ScrollView contentContainerStyle={styles.formContent}>
            ...
            <TextInput
              ...
              placeholder="Contale a la gente qué tipo de películas o series te gustan..."
            ...
            <Text style={[styles.label, { marginTop: theme.spacing.xl }]}>Tus títulos ({myItems.length}/5)</Text>

            {myItems.map((item) => (
              <View key={item.id} style={styles.itemRow}>
                {item.posterUrl || item.image ? (
                  <Image source={{ uri: item.posterUrl || getFileUrl(item, item.image) }} style={styles.itemThumb} />
                ) : (
                ...
                <View style={{ flex: 1 }}>
                  ...
                  <Text style={styles.itemRowYear}>
                    {[item.mediaType === 'tv' ? 'Serie' : item.mediaType === 'movie' ? 'Película' : null, item.year, item.director, item.genero]
                      .filter(Boolean)
                      .join(' · ')}
                  </Text>
                </View>
            ...
            {addingItem ? (
              <View style={styles.itemForm}>
                ...
                <View style={styles.searchRow}>
                  <TextInput
                    ...
                    editable={!savingItem}
                  />
                  <TouchableOpacity style={styles.searchBtn} onPress={handleSearchTmdb} disabled={searching || savingItem}>
                    {searching ? (
                ...
                {searchResults.length > 0 && (
                  <View style={styles.searchResults}>
                    {searchResults.map((result) => (
                      ...
                      >
                        {result.posterUrl ? (
                          <Image source={{ uri: result.posterUrl }} style={styles.itemThumb} />
                        ) : (
                          <View style={[styles.itemThumb, styles.itemThumbEmpty]}>
                        ...
                        <View style={{ flex: 1 }}>
                          <Text style={styles.itemRowTitle} numberOfLines={1}>{result.title}</Text>
                          <Text style={styles.itemRowYear}>
                            {[result.mediaType === 'tv' ? 'Serie' : 'Película', result.year].filter(Boolean).join(' · ')}
                          </Text>
                        </View>
              ...
              myItems.length < 5 && (
                <TouchableOpacity style={styles.addItemBtn} onPress={openNewItemForm}>
                  ...
                  <Text style={styles.addItemBtnText}>Agregar película o serie</Text>
                </TouchableOpacity>
              )
            )}
          </ScrollView>
...
const styles = StyleSheet.create({
  ...
  // 2:3, mismo motivo que cardImageWrapper en PeliculaDiscoverCard.
