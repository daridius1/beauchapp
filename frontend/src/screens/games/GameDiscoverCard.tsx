...
import { DiscoverGameProfile } from '../../services/gamesService';
import { CarouselDots } from '../../components/CarouselDots';

interface Props {
  profile: DiscoverGameProfile;
  onPrevProfile?: () => void;
  onNextProfile?: () => void;
...
// Carrusel de los videojuegos de la persona (imagen + título/año superpuestos) + su
// descripción + like/pase — mismo layout que TinderDiscoverCard, pero mostrando el
// perfil de videojuegos de alguien en vez de sus fotos.
export const GameDiscoverCard: React.FC<Props> = ({
  profile,
  onPrevProfile,
  onNextProfile,
  positionLabel,
        ...
        <View style={styles.profileCard}>
          <View style={styles.cardImageWrapper}>
            {items.length > 0 ? (
              <>
                <Image
                  source={{ uri: activeItem.coverUrl || getFileUrl(activeItem, activeItem.image) }}
                  style={styles.cardImage}
                />

                <View style={styles.itemOverlay} pointerEvents="none">
                  <Text style={styles.itemTitle}>{activeItem.title}</Text>
                  {(!!activeItem.year || !!activeItem.director || !!activeItem.genero) && (
                    <Text style={styles.itemYear}>
                      {[activeItem.year, activeItem.director, activeItem.genero].filter(Boolean).join(' · ')}
                    </Text>
                  )}
                </View>
              ...
              <View style={styles.emptyImage}>
                <FontAwesome name="gamepad" size={48} color="#404040" />
                <Text style={styles.emptyImageText}>Sin videojuegos subidos</Text>
              </View>
            )}
          </View>
...
const styles = StyleSheet.create({
  ...
  cardImageWrapper: {
    width: '100%',
    // 2:3 aprox. — mismo motivo que Películas: t_cover_big de IGDB (264x374) queda cerca
    // de 2:3, así el contenedor calza con la carátula real en vez de forzarla a cuadrado.
    aspectRatio: 2 / 3,
    borderRadius: 16,
    overflow: 'hidden',
