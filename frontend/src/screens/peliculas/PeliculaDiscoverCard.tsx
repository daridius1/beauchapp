...
import { DiscoverMovieProfile } from '../../services/moviesService';
import { CarouselDots } from '../../components/CarouselDots';

interface Props {
  profile: DiscoverMovieProfile;
  onPrevProfile?: () => void;
  onNextProfile?: () => void;
...
export const PeliculaDiscoverCard: React.FC<Props> = ({
  profile,
  onPrevProfile,
  onNextProfile,
  positionLabel,
        ...
        <View style={styles.profileCard}>
          <View style={styles.cardImageWrapper}>
            {items.length > 0 ? (
              <>
                <Image
                  source={{ uri: activeItem.posterUrl || getFileUrl(activeItem, activeItem.image) }}
                ...
                <View style={styles.itemOverlay} pointerEvents="none">
                  ...
                  <Text style={styles.itemYear}>
                    {[activeItem.mediaType === 'tv' ? 'Serie' : activeItem.mediaType === 'movie' ? 'Película' : null, activeItem.year, activeItem.director, activeItem.genero]
                      .filter(Boolean)
                      .join(' · ')}
                  </Text>
                </View>
              ...
              <View style={styles.emptyImage}>
                <Feather name="film" size={48} color="#404040" />
                <Text style={styles.emptyImageText}>Sin títulos subidos</Text>
              </View>
            )}
          </View>
...
const styles = StyleSheet.create({
  ...
  cardImageWrapper: {
    width: '100%',
    // 2:3 — mismo ratio en el que TMDB genera sus pósters, así el contenedor calza con la
    // imagen real en vez de forzarla a un cuadrado y tener que recortarla o dejarle barras.
    aspectRatio: 2 / 3,
    borderRadius: 16,
    overflow: 'hidden',
