...
import { booksService, BookItem, DiscoverBookProfile } from '../services/booksService';
import { openLibraryService, OpenLibraryResult } from '../services/openLibraryService';
...
export const BooksScreen: React.FC<Props> = ({ navigation }) => {
  const { user } = useAuth();
  const [activeTab, setActiveTab] = useState<'descubrir' | 'mis-libros' | 'matches'>('descubrir');

  // --- Mis Libros: se necesita en más de una pestaña (para saber si ya se puede dar like
  // en Descubrir), así que vive a nivel de pantalla.
  const [myItems, setMyItems] = useState<BookItem[]>([]);
  ...
  const fetchMyItems = async () => {
    ...
    try {
      ...
      setMyItems(await booksService.listMyItems(user.id));
    } catch (err) {
  ...
  // --- Pestaña Descubrir ---
  const [discoverProfiles, setDiscoverProfiles] = useState<DiscoverBookProfile[]>([]);
  ...
  const fetchDiscover = async () => {
    try {
      ...
      const feed = await booksService.getDiscoverFeed();
      setDiscoverProfiles(feed);
      setCurrentIndex((prev) => (feed.length > 0 ? prev % feed.length : 0));
    } catch (err) {
      console.error('Error cargando descubrimiento de libros:', err);
    } finally {
  ...
  const handleToggleLike = async () => {
    ...
    if (!target.isLiked && myItems.length === 0) {
      Toast.show({
        ...
        text2: 'Sube al menos un libro en "Mis Libros" antes de dar like.',
      });
      return;
    }
    ...
    try {
      if (target.isLiked) {
        if (target.likeId) await booksService.deleteLike(target.likeId);
      ...
      await booksService.createLike(user.id, target.user, true);
      ...
      const match = await booksService.getMatchBetweenUsers(idA, idB).catch(() => null);
      if (match) {
  ...
  const fetchProfile = async () => {
    if (!user) return;
    const profile = await booksService.getProfileByUserId(user.id);
  ...
  useFocusEffect(
    useCallback(() => {
      if (activeTab === 'mis-libros') fetchProfile();
      // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [activeTab, user?.id])
  );
  ...
  const handleSaveDescription = async () => {
    ...
    try {
      ...
      if (profileId) {
        await booksService.updateProfile(profileId, { description: description.trim() });
      } else {
        const created = await booksService.createProfile({ user: user.id, description: description.trim() });
  ...
  const openNewItemForm = () => {
    if (myItems.length >= 5) {
      Toast.show({ type: 'info', text1: 'Límite alcanzado', text2: 'Ya subiste el máximo de 5 libros.' });
      return;
    }
    setAddingItem(true);
    setSearchQuery('');
  ...
  const handleSearchBooks = async () => {
    ...
    setSearching(true);
    try {
      setSearchResults(await openLibraryService.search(q));
    } catch (err: any) {
  ...
  const handleSelectResult = async (result: OpenLibraryResult) => {
    if (!user) return;
    setSearchResults([]);
    setSearchQuery('');
    try {
      setSavingItem(true);
      await booksService.createItem({
        user: user.id,
        title: result.title,
  ...
  const handleDeleteItem = (item: BookItem) => setDeleteTarget(item);

  const confirmDeleteItem = async () => {
    if (!deleteTarget) return;
    try {
      await booksService.deleteItem(deleteTarget.id);
      fetchMyItems();
    } catch (err) {
  ...
  const fetchMatches = async () => {
    ...
    try {
      ...
      const res = await booksService.getMatchesList(user.id);
      setMatches(res.filter((m: any) => m.status !== 'unmatched'));
    } catch (err) {
      console.error('Error cargando matches de libros:', err);
    } finally {
  ...
  useEffect(() => {
    const sub = DeviceEventEmitter.addListener('onGlobalRefresh', async () => {
      await withMinimumDelay(async () => {
        ...
        else if (activeTab === 'mis-libros') await Promise.all([fetchMyItems(), fetchProfile()]);
  ...
  const confirmUnmatch = async () => {
    ...
    await booksService.unmatch(unmatchTarget.id, user.id);
  ...
  return (
    <View style={styles.container}>
      <View style={styles.tabHeader}>
        ...
        <TouchableOpacity style={[styles.tabBtn, activeTab === 'mis-libros' && styles.tabBtnActive]} onPress={() => setActiveTab('mis-libros')}>
          <Feather name="book" size={18} color={activeTab === 'mis-libros' ? theme.colors.primary : '#a3a3a3'} />
          <Text style={[styles.tabBtnText, activeTab === 'mis-libros' && styles.tabBtnTextActive]}>Mis Libros</Text>
        </TouchableOpacity>
      ...
      {activeTab === 'descubrir' &&
        (loadingDiscover ? (
          ...
          <View style={styles.emptyContainer}>
            <Feather name="book" size={40} color={theme.colors.textMuted} style={{ marginBottom: 12 }} />
            <Text style={styles.emptyText}>Todavía nadie ha compartido sus libros.</Text>
          </View>
        ) : (
          <BookDiscoverCard
        ...
        (loadingMyItems ? (
          ...
          <ScrollView contentContainerStyle={styles.formContent}>
            ...
            <TextInput
              ...
              placeholder="Contale a la gente qué tipo de libros te gustan..."
            ...
            <Text style={[styles.label, { marginTop: theme.spacing.xl }]}>Tus libros ({myItems.length}/5)</Text>

            {myItems.map((item) => (
              <View key={item.id} style={styles.itemRow}>
                {item.coverUrl ? (
                  <Image source={{ uri: item.coverUrl }} style={styles.itemThumb} />
                ) : (
            ...
            {addingItem ? (
              <View style={styles.itemForm}>
                ...
                <View style={styles.searchRow}>
                  <TextInput
                    ...
                    editable={!savingItem}
                  />
                  <TouchableOpacity style={styles.searchBtn} onPress={handleSearchBooks} disabled={searching || savingItem}>
                    {searching ? (
                      ...
                      >
                        ...
                        <View style={{ flex: 1 }}>
                          ...
                          <Text style={styles.itemRowYear} numberOfLines={1}>
                            {[result.author, result.year].filter(Boolean).join(' · ')}
                          </Text>
                        </View>
              ...
              myItems.length < 5 && (
                <TouchableOpacity style={styles.addItemBtn} onPress={openNewItemForm}>
                  ...
                  <Text style={styles.addItemBtnText}>Agregar libro</Text>
                </TouchableOpacity>
              )
            )}
          </ScrollView>
...
const styles = StyleSheet.create({
  ...
  // 2:3 — mismo criterio que Películas/Videojuegos, la carátula de un libro es portrait.
