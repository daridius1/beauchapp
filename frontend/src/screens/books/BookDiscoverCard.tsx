...
// Carrusel de los libros de la persona (carátula + título/autor superpuestos) + su
// descripción + like/pase — mismo layout que PeliculaDiscoverCard/GameDiscoverCard.
export const BookDiscoverCard: React.FC<Props> = ({
  profile,
  onPrevProfile,
  onNextProfile,
  positionLabel,
      ...
      <View style={styles.cardWrapper}>
        <View style={styles.profileCard}>
          <View style={styles.cardImageWrapper}>
            {items.length > 0 ? (
              <>
                <Image source={{ uri: activeItem.coverUrl }} style={styles.cardImage} />
              ...
              <View style={styles.emptyImage}>
                <Feather name="book" size={48} color="#404040" />
                <Text style={styles.emptyImageText}>Sin libros subidos</Text>
              </View>
            )}
          </View>
...
const styles = StyleSheet.create({
  ...
  cardImageWrapper: {
    width: '100%',
    // 2:3 aprox. — mismo criterio que Películas/Videojuegos: la carátula real de un libro
    // es portrait, así el contenedor calza en vez de forzarla a cuadrado.
    aspectRatio: 2 / 3,
    borderRadius: 16,
    overflow: 'hidden',
