...
export const ProblemsListScreen: React.FC<Props> = ({ navigation }) => {
  ...
  const fetchProblems = async (hideLoading = false) => {
    try {
      ...
      // 1. Obtener problemas
      let filterConditions: string[] = ['parent = null', 'deleted = false'];
      if (activeSearch) {
          ...
          problems.map(prob => {
            ...
            return (
              ...
              >
                ...
                <Text style={[styles.problemTitle, prob.deleted && { color: theme.colors.textMuted, fontStyle: 'italic' }]}>{prob.deleted ? '[Mensaje eliminado]' : prob.title}</Text>
