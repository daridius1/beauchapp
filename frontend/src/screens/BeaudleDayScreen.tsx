...
import { styles } from './beaudle/BeaudleDayScreen.styles';
...
export const BeaudleDayScreen: React.FC<Props> = ({ route, navigation }) => {
  ...
  const loadComments = async (statsId: string) => {
    try {
      ...
      console.error('Error cargando comentarios del Beaudle de ese día:', err);
    }
  };
  ...
  useEffect(() => {
    const subRefresh = DeviceEventEmitter.addListener('onGlobalRefresh', async () => {
      setLoading(true);
      await withMinimumDelay(() => fetchDay(true));
      setLoading(false);
    });
  ...
  const handleGuess = async (place: BeaudlePlace) => {
    ...
    setError(null);
    try {
      const data = await beaudleService.submitGuess(place.code, 'classic', day);
      setGame(data);
      if (data.status === 'won' && data.isToday) {
  ...
  const handleSendComment = async (content: string, photo: File | null) => {
    ...
    // El targetId es el registro compartido de stats del día (no un ID de "día"), así que
    // sin esto "En respuesta a Beaudle" en PostCard no tendría forma de saber a qué día
    // volver — se guarda el día explícitamente, igual que ya se hace al citar.
    formData.append('targetMeta', JSON.stringify({ day: game.day, dayNumber: game.dayNumber }));
  ...
  if (!game) {
    return (
      <View style={styles.loadingContainer}>
        <Text style={styles.errorText}>No se pudo cargar el Beaudle de ese día.</Text>
      </View>
    );
  }
  ...
  return (
    ...
    >
      <View style={styles.intro}>
        <Text style={styles.guessesRemaining}>
          {isInProgress
            ...
            : game.status === 'won' ? '¡Lo lograste!' : 'No lo lograste.'}
        </Text>
      ...
      {isInProgress && !game.isToday && (
        <View style={styles.pastDayNotice}>
          <Text style={styles.pastDayNoticeText}>
            Este es un Beaudle de un día pasado. Puedes responderlo, pero no cuenta para tu racha ni te da BeauTokens.
          </Text>
        </View>
      )}
        ...
        <>
          ...
          {user && (
            <EntityCommentBox
              ...
              placeholder="Comenta sobre este Beaudle..."
