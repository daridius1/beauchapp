...
import { songsService, SongRecord, DiscoverSongProfile } from '../services/songsService';
...
export const MusicaScreen: React.FC<Props> = ({ navigation }) => {
  const { user } = useAuth();
  // Cualquier canción sonando (fila de "Tus canciones", reproductor de edición) debe
  // callarse al salir de foco: al hacer push a otra pantalla (perfil, detalle) esta
  // pantalla no se desmonta, así que sin esto la canción seguía sonando de fondo.
  const isFocused = useIsFocused();
  const [activeTab, setActiveTab] = useState<'descubrir' | 'mis-canciones' | 'matches'>('descubrir');

  // --- Mis Canciones: se necesita en más de una pestaña (para saber si ya se puede dar
  // like en Descubrir), así que vive a nivel de pantalla.
  const [myItems, setMyItems] = useState<SongRecord[]>([]);
  ...
  const fetchMyItems = async () => {
    ...
    try {
      ...
      setMyItems(await songsService.listMyItems(user.id));
    } catch (err) {
  ...
  // --- Pestaña Descubrir ---
  const [discoverProfiles, setDiscoverProfiles] = useState<DiscoverSongProfile[]>([]);
  ...
  const fetchDiscover = async () => {
    try {
      ...
      const feed = await songsService.getDiscoverFeed();
      setDiscoverProfiles(feed);
      setCurrentIndex((prev) => (feed.length > 0 ? prev % feed.length : 0));
    } catch (err) {
      console.error('Error cargando descubrimiento de canciones:', err);
    } finally {
  ...
  const handleToggleLike = async () => {
    ...
    if (!target.isLiked && myItems.length === 0) {
      Toast.show({
        ...
        text2: 'Sube al menos una canción en "Mis Canciones" antes de dar like.',
      });
      return;
    }
    ...
    try {
      if (target.isLiked) {
        if (target.likeId) await songsService.deleteLike(target.likeId);
      ...
      await songsService.createLike(user.id, target.user, true);
      ...
      const match = await songsService.getMatchBetweenUsers(idA, idB).catch(() => null);
      if (match) {
  ...
  // --- Pestaña Mis Canciones: descripción de perfil ---
  ...
  const fetchProfile = async () => {
    if (!user) return;
    const profile = await songsService.getProfileByUserId(user.id);
  ...
  useFocusEffect(
    useCallback(() => {
      if (activeTab === 'mis-canciones') fetchProfile();
      // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [activeTab, user?.id])
  );
  ...
  const handleSaveDescription = async () => {
    ...
    try {
      ...
      if (profileId) {
        await songsService.updateProfile(profileId, { description: description.trim() });
      } else {
        const created = await songsService.createProfile({ user: user.id, description: description.trim() });
  ...
  // --- Formulario para agregar una canción. Ya no hay "editar" un ítem existente: como
  // elegir es solo buscar y tocar un resultado, "editar" no sería más que lo mismo que
  // borrar y agregar de nuevo — no aporta nada aparte, así que la única acción sobre un
  // ítem ya guardado es borrarlo. Elegir un resultado ya guarda, no hay botón "Guardar"
  // aparte (mismo motivo: no queda nada más que confirmar).
  ...
  const openNewItemForm = () => {
    if (myItems.length >= 5) {
      Toast.show({ type: 'info', text1: 'Límite alcanzado', text2: 'Ya subiste el máximo de 5 canciones.' });
      return;
    }
    setAddingItem(true);
    setSearchQuery('');
  ...
  const handleSearchSpotify = async () => {
    ...
    setSearching(true);
    try {
      setSearchResults(await spotifyService.search(q));
    } catch (err: any) {
  ...
  const handleSelectTrack = async (track: SpotifyTrackResult) => {
    if (!user) return;
    setSearchResults([]);
    setSearchQuery('');
    try {
      setSavingItem(true);
      await songsService.createItem({
        user: user.id,
        title: track.name,
        author: track.artist,
      ...
      Alert.alert('Error', err?.message || 'No se pudo guardar la canción.');
    } finally {
      setSavingItem(false);
    }
  };

  const [deleteTarget, setDeleteTarget] = useState<SongRecord | null>(null);

  const handleDeleteItem = (item: SongRecord) => setDeleteTarget(item);

  const confirmDeleteItem = async () => {
    if (!deleteTarget) return;
    try {
      await songsService.deleteItem(deleteTarget.id);
      fetchMyItems();
    } catch (err) {
  ...
  const fetchMatches = async () => {
    ...
    try {
      ...
      const res = await songsService.getMatchesList(user.id);
      setMatches(res.filter((m: any) => m.status !== 'unmatched'));
    } catch (err) {
      console.error('Error cargando matches de canciones:', err);
    } finally {
  ...
  useEffect(() => {
    const sub = DeviceEventEmitter.addListener('onGlobalRefresh', async () => {
      await withMinimumDelay(async () => {
        ...
        else if (activeTab === 'mis-canciones') await Promise.all([fetchMyItems(), fetchProfile()]);
  ...
  const confirmUnmatch = async () => {
    ...
    await songsService.unmatch(unmatchTarget.id, user.id);
  ...
  return (
    <View style={styles.container}>
      <View style={styles.tabHeader}>
        ...
        <TouchableOpacity style={[styles.tabBtn, activeTab === 'mis-canciones' && styles.tabBtnActive]} onPress={() => setActiveTab('mis-canciones')}>
          <Feather name="music" size={18} color={activeTab === 'mis-canciones' ? theme.colors.primary : '#a3a3a3'} />
          <Text style={[styles.tabBtnText, activeTab === 'mis-canciones' && styles.tabBtnTextActive]}>Mis Canciones</Text>
        </TouchableOpacity>
      ...
      {activeTab === 'descubrir' &&
        (loadingDiscover ? (
          ...
          <View style={styles.emptyContainer}>
            ...
            <Text style={styles.emptyText}>Todavía nadie ha compartido su música.</Text>
          </View>
        ) : (
          <MusicaDiscoverCard
        ...
        (loadingMyItems ? (
          ...
          <ScrollView contentContainerStyle={styles.formContent}>
            ...
            <TextInput
              ...
              placeholder="Contale a la gente qué tipo de música te gusta..."
            ...
            <Text style={[styles.label, { marginTop: theme.spacing.xl }]}>Tus canciones ({myItems.length}/5)</Text>
            ...
            {addingItem ? (
              <View style={styles.itemForm}>
                <Text style={styles.label}>Canción de Spotify</Text>
                ...
                <View style={styles.searchRow}>
                  <TextInput
                    ...
                    editable={!savingItem}
                  />
                  <TouchableOpacity style={styles.searchBtn} onPress={handleSearchSpotify} disabled={searching || savingItem}>
                    {searching ? (
                ...
                {searchResults.length > 0 && (
                  <View style={styles.searchResults}>
                    {searchResults.map((track) => (
                      ...
                      >
                        {track.imageUrl ? (
                          <Image source={{ uri: track.imageUrl }} style={styles.itemThumb} />
                        ) : (
                          <View style={[styles.itemThumb, styles.itemThumbEmpty]}>
                            <Feather name="music" size={16} color={theme.colors.textMuted} />
                          </View>
                        )}
                        <View style={{ flex: 1 }}>
                          <Text style={styles.itemRowTitle} numberOfLines={1}>{track.name}</Text>
                          <Text style={styles.itemRowYear} numberOfLines={1}>
              ...
              myItems.length < 5 && (
                <TouchableOpacity style={styles.addItemBtn} onPress={openNewItemForm}>
                  ...
                  <Text style={styles.addItemBtnText}>Agregar canción</Text>
                </TouchableOpacity>
              )
            )}
          </ScrollView>
...
const styles = StyleSheet.create({
  ...
  itemBlock: { paddingVertical: 8, borderBottomWidth: 1, borderBottomColor: theme.colors.border, gap: 8 },
