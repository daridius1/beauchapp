...
// Películas: categoría de referencia de "Conoce Beauchef" con like/match tipo Tinder
...
//   1. Para dar like hace falta tener al menos 1 película subida (acá sí se refuerza en el
...
// 1. Tope de 5 películas por usuario — el createRule de PocketBase no puede expresar un
...
onRecordCreateRequest((e) => {
    ...
    if (existing.length >= 5) {
        throw new BadRequestError("Ya subiste el máximo de 5 películas.");
    }
    return e.next();
}, "movie_items");
...
onRecordCreateRequest((e) => {
    ...
    // Regla nueva (no existe en Tinder Beauchef): hace falta tener al menos 1 película
    ...
    try {
        ...
        // No tiene ninguna película subida
    }
    if (!hasOwnItem) {
        throw new BadRequestError("Necesitas subir al menos una película a tu perfil antes de poder dar like.");
    }
    ...
    try {
        ...
        if (hasReciprocal) {
            const matchesCollection = $app.findCollectionByNameOrId("movie_matches");
        ...
        console.error("[Movies Match] Error al procesar match:", err);
    }
    return e.next();
}, "movie_likes");
...
onRecordAfterUpdateSuccess((e) => {
    try {
        ...
        console.error("[Movies Match] Error in onRecordAfterUpdateSuccess:", err.message || err);
    }
    return e.next();
}, "movie_matches");
...
onRecordAfterDeleteSuccess((e) => {
    try {
        ...
        console.error("[Movies Match] Error cleaning up likes on delete:", err.message || err);
    }
}, "movie_matches");
...
// 1 película no borrada, ya filtrado por match/bloqueo y ya marcado si le di like.
routerAdd("GET", "/api/movies/discover", (e) => {
    try {
        ...
        // Agrupar movie_items por usuario, excluyendo ya acá los que hicieron match o están
        ...
        console.error("[Movies Discover Route] Error:", err.message || err);
