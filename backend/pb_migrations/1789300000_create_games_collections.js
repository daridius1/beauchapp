...
migrate((app) => {
  ...
  // Categoría "Videojuegos" de "Conoce Beauchef", misma estructura que movie_items/movie_profiles/
  ...
  const profiles = new Collection({
    ...
    indexes: ["CREATE UNIQUE INDEX idx_game_profiles_user ON game_profiles (user)"],
  });
  app.save(profiles);
  ...
  const likes = new Collection({
    ...
    indexes: ["CREATE UNIQUE INDEX idx_game_from_to ON game_likes (fromUser, toUser)"],
  });
  app.save(likes);
  ...
  const matches = new Collection({
    ...
    indexes: ["CREATE UNIQUE INDEX idx_game_match_users ON game_matches (userA, userB)"],
  });
  app.save(matches);
}, (app) => {
  ["game_matches", "game_likes", "game_profiles", "game_items"].forEach((name) => {
