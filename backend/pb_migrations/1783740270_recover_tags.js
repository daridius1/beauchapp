...
migrate((app) => {
  ...
  console.log(`[RECOVERY] Recovered tags for ${recoveredPostsCount} posts.`);
  ...
  console.log(`[RECOVERY] Recovered tags for ${recoveredProblemsCount} problems.`);

  // 3. Limpiar registros de migraciones temporales en SQLite
  try {
    app.db().newQuery("DELETE FROM _migrations WHERE file IN ('1783740250_inspect.js', '1783740260_inspect.js')").execute();
    console.log("[RECOVERY] Purged temporary migrations from _migrations table.");
  } catch (err) {
    console.log("Error cleaning migrations table", err);
  }
}, (app) => {
