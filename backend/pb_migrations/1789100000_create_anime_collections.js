...
migrate((app) => {
  ...
  // Categoría "Anime" de "Conoce Beauchef", misma estructura que movie_items/movie_profiles/
  ...
  const profiles = new Collection({
    ...
    indexes: ["CREATE UNIQUE INDEX idx_anime_profiles_user ON anime_profiles (user)"],
  });
  app.save(profiles);
  ...
  const likes = new Collection({
    ...
    indexes: ["CREATE UNIQUE INDEX idx_anime_from_to ON anime_likes (fromUser, toUser)"],
  });
  app.save(likes);
  ...
  const matches = new Collection({
    ...
    indexes: ["CREATE UNIQUE INDEX idx_anime_match_users ON anime_matches (userA, userB)"],
  });
  app.save(matches);
}, (app) => {
  ["anime_matches", "anime_likes", "anime_profiles", "anime_items"].forEach((name) => {
